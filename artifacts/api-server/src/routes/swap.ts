...
// ── POST /api/swap/execute-approved ──────────────────────────────────────────
// Smart-wallet fallback: user has already called token.approve(relayer, amount)
// in a regular on-chain tx (smart wallets CAN execute regular txs with passkeys
// — only the off-chain EIP-2612 signature is incompatible).  We skip TX1
// (permit) and proceed directly to TX2 (transferFrom) → TX3 → TX4 → TX5.
const PreApprovedSwapSchema = z.object({
  tokenIn:     z.string().refine(isAddress, "invalid tokenIn address"),
  tokenOut:    z.string().refine(isAddress, "invalid tokenOut address"),
  amountIn:    z.string().regex(/^\d{1,13}$/, "amountIn must be 1–13 digit integer string"),
  owner:       z.string().refine(isAddress, "invalid owner address"),
...
router.post("/swap/execute-approved", async (req, res) => {
  const parsed = PreApprovedSwapSchema.safeParse(req.body);
  if (!parsed.success) {
    return res.status(400).json({ error: parsed.error.issues[0]?.message ?? "Invalid input" });
  }
  const { tokenIn, tokenOut, amountIn, owner, slippageBps } = parsed.data;

  const pair = resolveTokens(tokenIn, tokenOut);
  if (!pair) return res.status(400).json({ error: "Only USDC↔EURC swaps are supported" });
  ...
  // Verify user has pre-approved the relayer for at least amountIn
  const allowance = await publicClient.readContract({
    address:      pair.tokenIn,
    abi:          ERC20_ABI,
    functionName: "allowance",
  ...
  if (allowance < amountBig) {
    return res.status(400).json({
      error:     `Insufficient allowance: relayer approved for ${allowance.toString()} but swap needs ${amountBig.toString()}. Please approve first.`,
  ...
  if (balance < amountBig) return res.status(400).json({ error: "Insufficient token balance" });
  ...
  const amountOutMin = (freshQuote.amountOut * (10_000n - slipBps)) / 10_000n;
  const poolStable   = freshQuote.stable;
  const deadlineBig  = BigInt(Math.floor(Date.now() / 1000) + 1800);

  // TX2: pull tokens from user to relayer
  let pullHash: Hex;
  try {
    pullHash = await writeWithRetry(
      () => relayer.client.writeContract({
    ...
    return res.status(500).json({ error: `Token pull failed: ${msg}` });
  }

  // TX3 (conditional): ensure relayer→Aerodrome approval
  let needsApprove = true;
  try {
    const aeroAllowance = await publicClient.readContract({
      address:      pair.tokenIn,
      abi:          ERC20_ABI,
      functionName: "allowance",
      args:         [relayer.account.address, AERODROME_ROUTER],
    });
    needsApprove = aeroAllowance < amountBig;
  } catch {
    req.log.warn("Could not read Aerodrome allowance — will approve inline to be safe");
  }
  if (needsApprove) {
    try {
      const approveHash = await writeWithRetry(
        ...
        "TX3 approve (pre-approved path)",
      );
      await publicClient.waitForTransactionReceipt({ hash: approveHash, confirmations: 1 });
    } catch (err) {
      req.log.error({ err }, "execute-approved inline approve failed");
      return res.status(500).json({ error: "Could not approve Aerodrome router — swap aborted" });
    }
  }
  ...
  try {
    const preSwapQuote = await getBestAerodromeQuote(pair.tokenIn, pair.tokenOut, amountBig);
    if (preSwapQuote) swapAmountOutMin = (preSwapQuote.amountOut * (10_000n - slipBps)) / 10_000n;
  } catch (qErr) {
    req.log.warn({ qErr }, "pre-swap quote refresh failed — using original amountOutMin");
  }

  // Snapshot relayer tokenOut balance before swap
  let relayerBalBefore = 0n;
  try {
    relayerBalBefore = await publicClient.readContract({
      address:      pair.tokenOut,
    ...
    req.log.warn("Could not snapshot relayer tokenOut balance before swap");
  }
  ...
  try {
    ...
    try {
      const refundHash = await writeWithRetry(
        ...
        "TX4-recovery refund (pre-approved)",
      );
      req.log.info({ refundHash, owner, amountIn }, "refunded user after swap failure (pre-approved path)");
      return res.status(500).json({ error: "Swap reverted — your tokens have been refunded.", refundHash, pullHash });
    } catch (refundErr) {
      req.log.error({ refundErr, owner, amountIn }, "refund also failed — tokens stuck with relayer");
    }
    const raw   = err instanceof Error ? err.message : "Swap failed";
  ...
  let actualSwapOutput = swapAmountOutMin;
  try {
    const relayerBalAfter = await publicClient.readContract({
      address:      pair.tokenOut,
      abi:          ERC20_ABI,
    ...
    const received = relayerBalAfter - relayerBalBefore;
    if (received > 0n) actualSwapOutput = received;
  } catch {
    req.log.warn("Could not read relayer tokenOut balance after swap — falling back to swapAmountOutMin");
  }

  // TX5: send (actual output − protocol fee) to user
  const userAmount = (actualSwapOutput * (10_000n - SWAP_FEE_BPS)) / 10_000n;
  let transferHash: Hex;
  try {
    transferHash = await writeWithRetry(
      ...
      "TX5 transfer to user (pre-approved)",
    );
  } catch (err) {
    req.log.error({ err, swapHash }, "execute-approved TX5 (user transfer) failed");
    return res.status(500).json({ error: "Swap succeeded but output transfer failed. Contact support.", swapHash });
  }

  req.log.info(
    { pullHash, swapHash, transferHash, owner, tokenIn: pair.tokenIn, tokenOut: pair.tokenOut, amountIn, userAmount: userAmount.toString() },
    "pre-approved swap completed",
  );
